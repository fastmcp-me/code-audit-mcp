 ...
 */
export async function mcpCommand(subcommand = 'status', options = {}) {
    ...
    try {
        ...
        console.error(chalk.red('MCP command failed:'), error instanceof Error ? error.message : String(error));
        process.exit(1);
    }
}
/**
...
async function showStatus() {
    ...
    console.log(boxen(chalk.cyan.bold('🔌 MCP Configuration Status\n'), {
        padding: 1,
        margin: 1,
        borderStyle: 'round',
        borderColor: 'cyan',
    ...
    try {
        ...
        console.log(chalk.yellow('Executable Path:'), chalk.red('Not found - please install code-audit globally'));
    }
    ...
    for (const env of environments) {
        ...
        console.log(`  ${statusIcon} ${getEnvironmentDisplayName(env.environment)}: ${statusText}${existsText}`);
    ...
    console.log('  • Configure specific: ' +
        chalk.yellow('code-audit mcp configure --environment desktop'));
    console.log('  • Remove configuration: ' + chalk.yellow('code-audit mcp remove'));
...
async function configure(options) {
    ...
    if (options.environment) {
        ...
        const environments = mcpManager.getAvailableEnvironments();
        if (environments.length === 0) {
            console.log(chalk.yellow('No Claude environments detected on this system.'));
            return;
        }
        ...
        const choices = environments.map((env) => ({
            name: `${getEnvironmentDisplayName(env.environment)} ${env.configured ? '(reconfigure)' : ''}`,
...
async function backupConfigs() {
    ...
    try {
        ...
        const configCount = Object.keys(backup).filter((key) => key !== 'timestamp' && backup[key]).length;
 ...
 */
async function restoreConfigs(options) {
    if (!options.restore) {
    ...
    try {
        ...
        console.log(`  Configurations: ${Object.keys(backupContent)
        ...
        console.error(chalk.red('Failed to restore backup:'), error instanceof Error ? error.message : String(error));
        process.exit(1);
    }
}
/**
