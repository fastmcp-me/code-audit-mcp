...
async function pull_model_interactive(model_name) {
...
async function remove_model_interactive(model_name) {
    let target_model = model_name;
    if (!target_model) {
        const models = await getInstalledModels();
        if (models.length === 0) {
...
async function update_all_models() {
    ...
    const tasks = new Listr(models.map((model) => ({
...
export async function modelsCommand(options) {
    ...
    try {
        ...
        console.error(chalk.red('Models command failed:'), error instanceof Error ? error.message : String(error));
        process.exit(1);
    }
}
