...
export class CyclomaticComplexityCalculator {
     ...
     */
    static getDecisionPatterns(language) {
        const basePatterns = [
            /\bif\b/,
            /\belse\s+if\b/,
            /\bwhile\b/,
...
export class CognitiveComplexityCalculator {
     ...
     */
    static getNestingChange(line, language) {
        let change = 0;
...
export class NestingDepthCalculator {
     ...
     */
    static getDepthChange(line, language) {
        if (language === 'python') {
     ...
     */
    static calculatePythonDepthChange(line) {
...
export class ComplexityAnalyzer {
     ...
     */
    static countCommentLines(code, language) {
        const lines = code.split('\n');
        let commentLines = 0;
     ...
     */
    static getComplexityRating(complexity, type) {
        if (type === 'cyclomatic') {
            if (complexity <= 10)
                return 'Low';
            if (complexity <= 20)
     ...
     */
    static getMaintainabilityRating(index) {
        if (index >= 85)
            return 'Excellent';
        if (index >= 70)
            return 'Good';
