...
try {
  ...
  if (packageJson.name !== PACKAGE_NAME) {
    throw new Error(`Package name mismatch: expected ${PACKAGE_NAME}, got ${packageJson.name}`);
  }
  ...
  try {
    const scopedRegistry = execSync('npm config get @moikas:registry', { encoding: 'utf8' }).trim();
