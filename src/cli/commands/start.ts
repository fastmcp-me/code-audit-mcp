 ...
 */
function _isServerRunning(): boolean {
 ...
 */
function acquirePidLock(pid: number): boolean {
  const pidFile = getPidFilePath();
 ...
 */
function _savePid(pid: number): void {
  if (!acquirePidLock(pid)) {
    throw new Error(
      'Failed to acquire PID lock - another instance may be starting'
    );
  }
}
...
export async function startCommand(options: StartOptions): Promise<void> {
  ...
  try {
    ...
    if (error instanceof ConfigError) {
      ...
      console.log(
        chalk.gray('Run "code-audit config --reset" to restore defaults')
      );
      ...
      if (error.message.includes('timed out')) {
        ...
        console.log(
          chalk.gray('   • Check the configured host in your config')
        );
  ...
  if (!existsSync(serverPath)) {
    ...
    console.log(
      chalk.yellow('💡 Run "npm run build" to build the server first')
    );
  ...
  if (options.daemon) {
    ...
    child.on('error', (error) => {
      console.error(
        chalk.red('❌ Failed to start server daemon:'),
        error.message
      );
      process.exit(1);
    });
    ...
    if (isReady) {
      ...
      try {
        ...
        console.error(
          chalk.red('❌ Server process died shortly after starting')
        );
    ...
    process.on('SIGINT', async () => {
      ...
      try {
        ...
        const gracefulTimeout =
          config.server?.shutdown?.gracefulTimeout || 5000;
        ...
        const gracefulTimer = setTimeout(() => {
          console.log(
            chalk.yellow('⚠️  Graceful shutdown taking longer than expected...')
          );
        }, gracefulTimeout);
