...
function get_package_info(): PackageInfo {
  try {
...
async function install_update(package_name: string): Promise<void> {
...
function display_no_update_info(current: string): void {
  console.log(
  ...
  try {
    ...
    if (!has_update && !force) {
      if (is_same) {
        ...
        console.log(
          chalk.yellow('⚠️  Local version is newer than published version')
        );
        console.log(
          `Local: ${chalk.bold(pkg.version)}, Published: ${chalk.dim(latest_version)}`
        );
      }
      return;
    }
    ...
    try {
      ...
      console.log(
        chalk.dim('Restart your terminal to use the updated version')
      );
    } catch (error) {
      console.error(
        chalk.red('\n❌ Update failed:'),
    ...
    console.error(
      chalk.red('Update command failed:'),
      error instanceof Error ? error.message : String(error)
    );
    process.exit(1);
  }
