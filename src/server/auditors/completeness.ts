...
export class CompletenessAuditor extends BaseAuditor {
   ...
   */
  private isFixmeComment(line: string): boolean {
    const fixmePatterns = [
      /\/\/.*fixme/i,
      /\/\*.*fixme.*\*\//i,
      /#.*fixme/i,
   ...
   */
  private isPlaceholderImplementation(line: string): boolean {
   ...
   */
  private isUnhandledPromise(line: string, language: string): boolean {
   ...
   * Create TODO issue
   */
  private createTodoIssue(line: string, lineNumber: number): AuditIssue {
    return {
      id: `todo_${lineNumber}`,
   ...
   * Create FIXME issue
   */
  private createFixmeIssue(line: string, lineNumber: number): AuditIssue {
    return {
   ...
   * Create HACK issue
   */
  private createHackIssue(line: string, lineNumber: number): AuditIssue {
    return {
      id: `hack_${lineNumber}`,
      location: { line: lineNumber },
      severity: 'medium',
   ...
   */
  private createPlaceholderIssue(line: string, lineNumber: number): AuditIssue {
    return {
