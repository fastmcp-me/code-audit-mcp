...
export class PerformanceAuditor extends BaseAuditor {
   ...
   */
  private detectPerformancePatterns(code: string, language: string): AuditIssue[] {
   ...
   */
  private isNestedLoop(line: string, lines: string[], index: number, language: string): boolean {
    const loopPatterns = [
      /\bfor\s*\(/,
      /\bwhile\s*\(/,
      /\.forEach\s*\(/,
   ...
   */
  private isInefficientStringConcatenation(line: string, language: string): boolean {
   ...
   */
  private isQueryInLoop(line: string, lines: string[], index: number, language: string): boolean {
    const queryPatterns = [
      /\.query\s*\(/,
      /\.execute\s*\(/,
      /SELECT\s+/i,
   ...
   */
  private isObjectCreationInLoop(line: string, lines: string[], index: number, language: string): boolean {
   ...
   */
  private isBlockingInAsyncContext(line: string, lines: string[], index: number, language: string): boolean {
    if (language !== 'javascript' && language !== 'typescript') {
      return false;
    }
  ...
  private createStringConcatenationIssue(line: string, lineNumber: number): AuditIssue {
    return {
  ...
  private createSyncFileOperationIssue(line: string, lineNumber: number): AuditIssue {
    return {
  ...
  private createObjectCreationInLoopIssue(line: string, lineNumber: number): AuditIssue {
    return {
  ...
  private createMissingCachingIssue(line: string, lineNumber: number): AuditIssue {
    return {
  ...
  private createInefficientArrayOperationIssue(line: string, lineNumber: number): AuditIssue {
    return {
  ...
  private createBlockingOperationIssue(line: string, lineNumber: number): AuditIssue {
    return {
  ...
  private createDOMOperationIssue(line: string, lineNumber: number): AuditIssue {
    return {
   ...
   * Adjust severity based on performance criticality
   */
  private adjustSeverityForContext(issues: AuditIssue[], context?: any): AuditIssue[] {
